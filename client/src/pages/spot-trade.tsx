...
export default function SpotTrade({ pair: initialPair }: { pair: string }) {
  ...
  return (
    <div className="h-screen flex flex-col bg-background" data-testid="page-spot-trade">
      <header className="flex items-center justify-between gap-2 px-4 py-2 border-b border-border bg-background flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <h1 className="text-lg font-bold" data-testid="text-active-pair">{activePair.replace("USDT", "/USDT")}</h1>
            <span className={`text-sm font-semibold ${parseFloat(priceChange) >= 0 ? "text-green-600" : "text-red-500"}`} data-testid="text-price-change">
        ...
        <div className="w-80 border-l border-border flex flex-col bg-background overflow-hidden">
          ...
          <div className="border-t border-border max-h-48 overflow-y-auto">
            <div className="px-3 py-1.5 border-b border-border">
              <span className="text-xs font-semibold text-foreground">Order History</span>
            </div>
            {orderHistory.length === 0 ? (
              <div className="p-3 text-xs text-muted-foreground text-center">No orders yet</div>
            ) : (
              orderHistory.slice(0, 10).map(order => (
                <div key={order.id} className="flex items-center justify-between px-3 py-1.5 text-[11px] border-b border-border/50" data-testid={`row-order-${order.id}`}>
                  <div>
